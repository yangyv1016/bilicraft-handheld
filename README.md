...
官方插件市场列表维护在仓库内的静态索引文件：

- [`plugin-market/index.json`](plugin-market/index.json)
- [`plugin-market/index.example.jsonc`](plugin-market/index.example.jsonc)：带字段备注的 JSONC 示例，不作为 App 正式市场数据

开发者发布插件时，先把 `.bhplugin` 上传到稳定下载地址，再通过 PR 修改正式索引文件，填写插件 ID、版本、下载地址、SHA-256、权限和更新说明。没有真实 `.bhplugin` 下载地址时，不要把占位条目写入 `plugin-market/index.json`。
