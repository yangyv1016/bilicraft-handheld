...
enum class PacketKey {
    // --- serverbound ---
    SB_LOGIN_START,
    SB_ENCRYPTION_RESPONSE,
    SB_LOGIN_ACK,             // 1.20.2+ 登录完成确认（进入 configuration）
    SB_CONFIG_KNOWN_PACKS,    // 1.20.5+/协议766+ 回应服务器 Known Packs
    SB_CONFIG_FINISH_ACK,     // 确认 configuration 结束（进入 play）
    SB_CONFIG_KEEP_ALIVE,
    SB_KEEP_ALIVE_PLAY,
    SB_CHAT_MESSAGE,          // play 阶段发送聊天
    SB_CHAT_SESSION_UPDATE,   // 1.19.3+ session 体系：上报玩家公钥/会话
    ...
    CB_SET_COMPRESSION,
    CB_LOGIN_DISCONNECT,
    CB_CONFIG_KNOWN_PACKS,    // 1.20.5+/协议766+ 服务器询问 Known Packs
    CB_CONFIG_FINISH,
    CB_CONFIG_KEEP_ALIVE,
    CB_CONFIG_DISCONNECT,
    CB_PLAY_DISCONNECT,
    CB_KEEP_ALIVE_PLAY,
    CB_SYSTEM_CHAT,           // 系统消息（服务器广播、命令回显等，多数聊天走这里）
    CB_PLAYER_CHAT,           // 玩家签名聊天（结构复杂，这里只提取可读文本）
}
...
data class PacketPalette(
    ...
    private val cbReverse: Map<Int, PacketKey> = cbMap.entries.associate { (k, v) -> v to k }

    /** 逻辑包 → 发送 id。未登记的包返回 null（调用方据此决定是否发送）。 */
    fun sbId(key: PacketKey): Int? = sbMap[key]

    /** 接收 id → 逻辑包。未识别返回 null（忽略该包）。 */
    fun cbKey(id: Int): PacketKey? = cbReverse[id]
}
